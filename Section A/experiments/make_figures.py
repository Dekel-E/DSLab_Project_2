...
HERE = Path(__file__).resolve().parents[1]  # Section A directory
...
def fig_tried():
    ...
    lec = pd.read_csv(RES / "lecture_variants.csv")
    ...
    items = [
        ...
        ("Diversity batches (rejected: not\nreproducible across sklearn versions)",
                                            mean_of(h2h, "cfg", "C_div_repcv")
        ...
        ("Density-weighted selection",      mean_of(lec, "selection", "density_b0.5")
                                             - mean_of(lec, "selection", "uncertainty")),
        ("Query-by-Committee (3 RFs)",      mean_of(lec, "selection", "qbc")
                                             - mean_of(lec, "selection", "uncertainty")),
...
def fig_cumulative():
    ...
    stages = [
        ...
        ("+ repeated-CV ratio pick", fb[fb["variant"] == "final: uncertainty + rebalance"]["f1"].mean()),
    ]
    fig, ax = new_ax(
        "From skeleton to final: +0.23 F1",
...
def fig_final():
    ...
    order = ["initial 500 only", "random 5000", "uncertainty 5000",
             "final: uncertainty + rebalance"]
    names = ["initial 500 only", "random full budget", "uncertainty sampling",
             "final: + rebalance (CV ratio)"]
    fig, ax = new_ax(
        "Final per-seed results — mean F1 = 0.642",
        "F1(Left) on the local test sets · identical across scikit-learn 1.8 / 1.9 · every stage improves every seed",
    )
